...
public class App {
	...
	public static void main(String[] args) {
		...
		if (cmd.hasOption(CMD_OPTION_BLUR)) {
			filter = new BlurFilter();
		}

		if (cmd.hasOption(CMD_OPTION_PREWITT)) {
			filter = new PrewittFilter(filter);
		} else {
			filter = new SobelFilter(filter);
		}

		if (cmd.hasOption(CMD_OPTION_THRESHOLD)) {
			filter = new LowerThresholdFilter(Integer.parseInt(cmd.getOptionValue(CMD_OPTION_THRESHOLD)), filter);
		}

		BufferedImage destImage = filter.applyFilter(sourceImage);
		try {
			File destFile = new File(cmd.getOptionValue(CMD_OPTION_OUT_FILE));
			ImageIO.write(destImage, "png", destFile);
		} catch (IOException myException) {
			System.out.println("Cannot write output image: ");
