...
public class Menu extends JMenuBar {
	...
	/**
	 * Load all SWT1-Plugins (that implement the interface JmjrstPlugins)
	 * 
	 * @see org.jis.plugins.JmjrstPlugin JmjrstPlugins
	 * 
	 * @return A menu containing two MenuItems for each JmjrstPlugin (one to run it and one to configure it).
	 */
	private JMenu createSwt1PluginMenu(Main m) {
		// SWT1 Plugin Menu
		JMenu swt1Plugins = new JMenu("SWT1-Plugins");
		...
		while (iter.hasNext()) {
			...
			// add menu item for plugin configuration if available
			if (plugin.isConfigurable()) {
				JMenuItem cmi = createConfigMenuItem(plugin);
				swt1Plugins.add(cmi);
			}

			// add separator if there is another plugin
			if (iter.hasNext()) {
				swt1Plugins.addSeparator();
			}
